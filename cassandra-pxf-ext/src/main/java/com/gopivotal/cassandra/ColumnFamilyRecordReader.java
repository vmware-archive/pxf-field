...
public class ColumnFamilyRecordReader extends
		RecordReader<ByteBuffer, SortedMap<ByteBuffer, IColumn>>
		implements
		org.apache.hadoop.mapred.RecordReader<ByteBuffer, SortedMap<ByteBuffer, IColumn>> {
	...
	public void initialize(InputSplit split, TaskAttemptContext context)
		...
		try {
			...
			client = ColumnFamilyInputFormat.createAuthenticatedClient(
					location, port, conf);
	...
	public boolean nextKeyValue() throws IOException {
		if (!iter.hasNext()) {
			logger.debug("Finished scanning " + iter.rowsRead()
					+ " rows (estimate was: " + totalRowCount + ")");
			return false;
		}
	...
	private abstract class RowIterator extends
		...
		private RowIterator() {
			try {
				partitioner = FBUtilities.newPartitioner(client
		...
		private IColumn unthriftifyCounter(CounterColumn column) {
			// CounterColumns read the counterID from the System table, so need
	...
	private class StaticRowIterator extends RowIterator {
		...
		private void maybeInit() {
			...
			if (totalRead == 0) {
				...
				startToken = partitioner.getTokenFactory().toString(
						partitioner.getToken(Iterables.getLast(rows).key));
			...
			KeyRange keyRange = new KeyRange(batchSize)
					...
					.setEnd_token(split.getEndToken()).setRow_filter(filter);
			try {
				...
				if (isEmptyPredicate) {
					...
					// all ghosts, spooky
					if (rows.isEmpty()) {
						// maybeInit assumes it can get the start-with key from
						// the rows collection, so add back the last
						rows.add(ks);
						maybeInit();
						return;
					}
		...
		protected Pair<ByteBuffer, SortedMap<ByteBuffer, IColumn>> computeNext() {
			...
			SortedMap<ByteBuffer, IColumn> map = new TreeMap<ByteBuffer, IColumn>(
					comparator);
	...
	private class WideRowIterator extends RowIterator {
		...
		private void maybeInit() {
			...
			try {
				...
				for (KeySlice row : rows)
					n += row.columns.size();
				logger.debug(
						"read {} columns in {} rows for {} starting with {}",
						new Object[] { n, rows.size(), keyRange, lastColumn });

				wideColumns = Iterators.peekingIterator(new WideColumnIterator(
						rows));
				if (wideColumns.hasNext()
						&& wideColumns.peek().right.keySet().iterator().next()
								.equals(lastColumn))
					wideColumns.next();
				if (!wideColumns.hasNext())
					rows = null;
		...
		protected Pair<ByteBuffer, SortedMap<ByteBuffer, IColumn>> computeNext() {
			...
			Pair<ByteBuffer, SortedMap<ByteBuffer, IColumn>> next = wideColumns
					.next();
		 ...
		 * 
		 * @param next
		 *            just fetched row slice
		 */
		private void maybeIncreaseRowCounter(
			...
			protected Pair<ByteBuffer, SortedMap<ByteBuffer, IColumn>> computeNext() {
				while (true) {
					if (columns.hasNext()) {
						...
						IColumn column = unthriftify(cosc);
						ImmutableSortedMap<ByteBuffer, IColumn> map = ImmutableSortedMap
								.of(column.name(), column);
						return Pair
								.<ByteBuffer, SortedMap<ByteBuffer, IColumn>> create(
										currentRow.key, map);
					}
	...
	// now.
	public boolean next(ByteBuffer key, SortedMap<ByteBuffer, IColumn> value)
			throws IOException {
